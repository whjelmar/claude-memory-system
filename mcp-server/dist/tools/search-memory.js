...
export const searchMemoryTool = {
    name: "memory_search",
    description: "Searches across memory files for the given query. Can search in sessions, decisions, knowledge, or all areas. Returns matching excerpts with file paths.",
    inputSchema: {
        type: "object",
        properties: {
            query: {
            ...
            scope: {
                ...
                description: "Where to search: 'sessions' (session logs), 'decisions' (decision records), 'knowledge' (knowledge files), or 'all' (everywhere)",
            },
        },
 ...
 */
export async function executeSearchMemory(params) {
    ...
    if (params.scope === "all") {
        ...
        results.totalMatches += rootResults.reduce((sum, r) => sum + r.matches.length, 0);
    }
    // Search each area
    for (const area of searchAreas) {
        const areaResults = await searchInFiles(area.path, params.query, ".md");
        results.results[area.key].push(...areaResults);
        results.totalMatches += areaResults.reduce((sum, r) => sum + r.matches.length, 0);
    }
    return results;
}
/**
 ...
 */
export function formatSearchMemoryResult(result) {
    const lines = [];
