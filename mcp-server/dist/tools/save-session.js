...
import { getMemoryPath, ensureDir, writeFileSafe, getTimestamp, getDateString, } from "../utils/file-helpers.js";
/**
 * Tool definition for memory_save_session
 */
export const saveSessionTool = {
    name: "memory_save_session",
    description: "Saves the current session to memory. Creates a timestamped session file with work completed, decisions made, discoveries, and next steps. Also updates the current_context.md file.",
    inputSchema: {
        type: "object",
        properties: {
            summary: {
 ...
 */
function generateSessionContent(params, timestamp) {
    const lines = [];
 ...
 */
function generateContextContent(params, timestamp) {
    const lines = [];
 ...
 */
export async function executeSaveSession(params) {
 ...
 */
export function formatSaveSessionResult(result) {
    return [
